...
begin
  ...
  response = RestClient.post("#{SERVER}/users/sign_in.json", params)
  ...
  response = RestClient.get "#{SERVER}/api/observations.json?all=true", {content_type: :json, accept: :json, 'X-User-Email' => 'bjorn@biocaching.com', 'X-User-Token' => token}
  puts response.code
