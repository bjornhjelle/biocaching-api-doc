...
if ARGV.size < 3
  puts "usage:"
  puts "  ruby #{$0} server|localhost <id of observation to update>  <filename of picture to add>"
  puts 
  exit 1
else
  text   = ARGV[1]
...
begin
  ...
  puts "will add picture to observation with id: %d" % text
  ...
  response = RestClient.post "#{@server}/observations/#{text}/pictures", picture_params, @http_headers
